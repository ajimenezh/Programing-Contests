...
int val1[11] = {0,4,7,10,14,20,25,26,29,33,35};
int val2[11] = {0,5,8,11,15,21,26,27,30,33,36};
...
public:
    ...
    vector<string> nextDay(vector<int> snowFalls) {
        ...
        for (int i=0; i<n; i++) if (!people[i].check()) {
            ...
            for (int p=0; p<boardSize; p++) {
                for (int r=0; r<boardSize; r++) {
                    ...
                    if ((people[i].x>=boardSize/2 && p<boardSize/2) || (people[i].y>=boardSize/2 && r<boardSize/2)) continue;
                    if ((people[i].x<boardSize/2 && p>=boardSize/2) || (people[i].y<boardSize/2 && r>=boardSize/2)) continue;
        ...
        for (int i=0; i<boardSize; i++) for (int j=0; j<boardSize; j++) if (mapa[i][j]) {
            ...
            if (cnt<total && isPerson[i/factor][j/factor]==0 && 30*snowFine>salary && place[px][py]<total/4) {

                //cerr<<px<<" "<<py<<" "<<i<<" "<<boardSize<<endl;

                place[px][py]++;

                px = px*(boardSize/2) + (boardSize/4);
                py = py*(boardSize/2) + (boardSize/4);
