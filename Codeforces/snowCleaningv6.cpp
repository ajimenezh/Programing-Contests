...
int val1[11] = {0,4,7,10,16,20,48,32,32,33,35};
int val2[11] = {0,5,8,11,16,21,48,32,32,33,36};
...
public:
    ...
    struct worker {
        ...
        void update() {
            ...
            if (!moves.empty() && !doneMoving) {
                ...
                if (next==0 || next==2) {
                    if (tenx == 0 && teny == 1) {
                        if (x+dirx[next]==ox+factor || x+dirx[next]>=boardSize) {
                            tenx = 2;
                            if (oy==boardSize-1) next = tenx;
                            else if (y+diry[1]==oy+factor || y+diry[1]>=boardSize) {
                                teny = 3;
                                next = 3;
                            }
                            else {
                ...
                if (oy==boardSize-1) {
                    ...
                    else if (x+dirx[0]==ox+factor || x+dirx[0]==boardSize) {
                        next = 2;
                        tenx = 2;
                    }
                    else {
        ...
        bool check() {
            ...
            return 0;
            for (int i=ox; i<min(ox+factor,boardSize); i++) {
                for (int j=oy; j<min(oy+factor,boardSize); j++) {
                    if (mapa[i][j]) {
                        return 1;
                    }
                }
    ...
    int init(int _boardSize, int _salary, int _snowFine) {
        ...
        //if (snowFine-salary<10) factor = 4;

        total = (salary>50?val1[boardSize/5]:val2[boardSize/5]);
    ...
    void makeMoves(int i, int x, int y) {
        ...
        //cerr<<"Done!"<<" "<<i<<" "<<people[i].moves.size()<<" "<<tmp.size()<<endl;
        people[i].moves = tmp;
    ...
    vector<string> nextDay(vector<int> snowFalls) {
        ...
        for (int i=0; i<n; i++) if (!people[i].check()) {
            ...
            for (int p=0; p<boardSize; p++) {
                for (int r=0; r<boardSize; r++) {
                    if (!mapa[p][r] || isPerson[p/factor][r/factor]) continue;
                    if (p==people[i].x && r==people[i].y) continue;
                    if (people[i].x/(boardSize/4)!=p/(boardSize/4) && !(people[i].x/(boardSize/4)>=3 && p/(boardSize/4)>=3)) continue;
                    if (people[i].y/(boardSize/4)!=r/(boardSize/4) && !(people[i].y/(boardSize/4)>=3 && r/(boardSize/4)>=3)) continue;
                    if (abs(people[i].x-p)+abs(people[i].y-r)<d) {
                        d = abs(people[i].x-p)+abs(people[i].y-r);
                        px = p;
                        py = r;
                    }
                }
            ...
            isPerson[people[i].x][people[i].y] = 0;
            makeMoves(i,px, py);
        ...
        for (int i=0; i<boardSize; i++) for (int j=0; j<boardSize; j++) if (mapa[i][j]) {
            ...
            if (cnt<total && isPerson[i/factor][j/factor]==0 && 30*snowFine>salary && place[px][py]<total/16) {

                //cerr<<px<<" "<<py<<endl;

                place[px][py]++;

                px = px*(boardSize/4) + (boardSize/8);
                py = py*(boardSize/4) + (boardSize/8);

                people.push_back(worker(px,py));
                stringstream ss;
                ss<<"H ";
        ...
        for (int k=0; k<n; k++) {
            if (people[k].check()) {
                ...
                if (people[k].next>3) people[k].next = 0;
                if (people[k].x+dirx[people[k].next]==boardSize) {
                    people[k].next = 2;
                }
                else if (people[k].y+diry[people[k].next]==boardSize) {
                    people[k].next = 3;
                }
                else if (people[k].x+dirx[people[k].next]==-1) {
                    people[k].next = 0;
                }
                else if (people[k].y+diry[people[k].next]==-1) {
                    people[k].next = 1;
                }
