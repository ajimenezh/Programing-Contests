...
public:
    ...
    struct worker {
        ...
        void update() {
            ...
            if (!moves.empty() && !doneMoving) {
                ...
                if (x+dirx[next]>=0 && x+dirx[next]<boardSize) x += dirx[next];
                else next = (next+1)%4;
                if (y+diry[next]>=0 && y+diry[next]<boardSize) y += diry[next];
                else next = (next+1)%4;
                ...
                if (next==0 || next==2) {
                    if (tenx == 0 && teny == 1) {
                        if (x+dirx[next]==ox+5 || x+dirx[next]>=boardSize) {
                            tenx = 2;
                            if (y+diry[1]==oy+5 || y+diry[1]>=boardSize) {
                                teny = 3;
                                next = 3;
                            }
                            else {
    ...
    vector<string> nextDay(vector<int> snowFalls) {
        ...
        int n = people.size();
        for (int i=0; i<n; i++) people[i].busy = 0;

        for (int i=0; i<boardSize; i++) for (int j=0; j<boardSize; j++) if (mapa[i][j]) {
            int possibleWorker = nearest(i, j);
            if (possibleWorker!=-1 && isPerson[i/5][j/5]==0) {
                isPerson[people[possibleWorker].ox/5][people[possibleWorker].ox/5] = 0;
                makeMoves(possibleWorker,i, j);
                people[possibleWorker].beginMoving();
                isPerson[i/5][j/5] = 1;
                continue;
            }
            else if (cnt<99 && isPerson[i/5][j/5]==0 && 30*snowFine>salary) {
