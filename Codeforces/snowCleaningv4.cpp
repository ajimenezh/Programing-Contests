...
public:
    ...
    vector<string> nextDay(vector<int> snowFalls) {
        ...
        for (int i=0; i<n; i++) if (!people[i].check()) {
            ...
            isPerson[people[i].ox/factor][people[i].oy/factor] = 0;
            makeMoves(i,px, py);
            people[i].beginMoving();
            isPerson[px/factor][py/factor] = 1;

        }//cerr<<"Done"<<endl;

        for (int i=0; i<boardSize; i++) for (int j=0; j<boardSize; j++) if (mapa[i][j]) {
            if (cnt<(salary>50?val1[boardSize/5]:val2[boardSize/5]) && isPerson[i/factor][j/factor]==0 && 30*snowFine>salary) {
